...
/*echo $org = $_POST["organization_name"];
echo $dep = $_POST["department_name"];
echo $dep2 = $_POST["department2_name"];*/
...
$sql_idcard = "SELECT * FROM `user` WHERE $id_card";
$result1 = mysqli_query($conn,$sql_idcard);
    if ($num = $result1->num_rows > 0) {
      header("location:create.php");
    }
    else{
      echo $sql_email = "SELECT * FROM `user` WHERE $email";
      $result2 = mysqli_query($conn,$sql_email);
